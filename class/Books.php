...
class Books {	
	...
	public function listBook(){		
		...
		$sqlQuery = "SELECT book.bookid, book.picture, book.name, book.status, book.isbn, book.no_of_copy, book.updated_on, author.name as author_name, category.name AS category_name, rack.name As rack_name, publisher.name AS publisher_name 
			...
			if(!empty($searchValue)){
				$sqlQuery .= ' WHERE (book.bookid LIKE "%'.$searchValue.'%" ';
				$sqlQuery .= ' OR book.name LIKE "%'.$searchValue.'%" ';
				$sqlQuery .= ' OR book.status LIKE "%'.$searchValue.'%" ';
				$sqlQuery .= ' OR book.isbn LIKE "%'.$searchValue.'%" ';
				$sqlQuery .= ' OR author.name LIKE "%'.$searchValue.'%" ';
				$sqlQuery .= ' OR category.name LIKE "%'.$searchValue.'%" ';
				$sqlQuery .= ' OR rack.name LIKE "%'.$searchValue.'%" ';
				$sqlQuery .= ' OR publisher.name LIKE "%'.$searchValue.'%" )';
			}
	...
	public function insert(){
		if($this->name && $_SESSION["userid"]) {
			$stmt = $this->conn->prepare("
				INSERT INTO ".$this->bookTable."(`name`, `status`, `isbn`, `no_of_copy`, `categoryid`, `authorid`, `rackid`, `publisherid`, `picture`)
				VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?)");
			...
			// Upload and store the book photo
			if ($_FILES['photo']['error'] === UPLOAD_ERR_OK) {
				...
				$picture = 'default.jpg'; // Set a default photo if no photo is uploaded
			}
			
			$stmt->bind_param("sssiiiiis", $this->name, $this->status, $this->isbn, $this->no_of_copy, $this->category, $this->author, $this->rack, $this->publisher, $picture);
