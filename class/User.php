...
class User {	
	...
	public function listUsers(){		
		...
		if(!empty($searchValue)){
			...
			$sqlQuery .= ' OR last_name LIKE "%'.$searchValue.'%" ';
			$sqlQuery .= ' OR first_name LIKE "%'.$searchValue.'%" ';
			$sqlQuery .= ' OR email LIKE "%'.$searchValue.'%" ';
			$sqlQuery .= ' OR password LIKE "%'.$searchValue.'%" ';
			$sqlQuery .= ' OR role LIKE "%'.$searchValue.'%" ';
			$sqlQuery .= ')';
	...
	public function insert(){		
		if($this->role && $this->email && $this->password && $_SESSION["userid"]) {
			...
			$mail->Body    = 'Bienvenue '.$this->first_name.'dans la bibliothe IGA! Plongez dans un océan de connaissances et
			laissez les mots vous guider vers de nouveaux horizons litteraires. '; // Replace with the desired message body for the notification
